...
/**
 * Created by Sarthak on 9/10/2018.
 */

public class GoldMineralDetector extends OpenCVPipeline {

    public enum GenericDetectionMode {
        PERFECT_AREA, MAX_AREA
    }

    public enum GoldMinearlPosition{
        LEFT, CENTER, RIGHT
    }
    ...
    public Mat processFrame(Mat rgba, Mat gray) {
        ...
        colorFilter.process(yellowConvert,maskYellow);

        if(stretch){
            structure = Imgproc.getStructuringElement(Imgproc.CV_SHAPE_RECT,stretchKernal);
            Imgproc.morphologyEx(maskYellow,maskYellow, Imgproc.MORPH_CLOSE,structure);
        }
        List<MatOfPoint> contours = new ArrayList<>();

        Imgproc.findContours(maskYellow, contours, hiarchy, Imgproc.RETR_TREE, Imgproc.CHAIN_APPROX_SIMPLE);
        Imgproc.drawContours(workingMat, contours, -1, new Scalar(255,215,0), 2);
        ...
        for (MatOfPoint c : contours) {
            ...
            if (debugContours && area > 100) {
                Imgproc.circle(workingMat, centerPoint, 3, new Scalar(255,215,0), 3);
                Imgproc.putText(workingMat, "Area: " +  String.format("%.1f", area), centerPoint, 0, 0.5, new Scalar(255,215,0));
            }
